...
import 'package:wordpress_app/services/remote_config.dart';
    ...
    if (userName == commentUser) {
      AppService().checkInternet().then((hasInternet) async{
        if (!hasInternet!) {
          ...
          setState(() => _isSomethingChanging = true);
          await RemoteConfigService().getCredentials().then((List<String> credentials){
            if(credentials.isNotEmpty){
              AuthService().customAuthenticationViaJWT(credentials[0], credentials[1]).then((JwtStatus? status) {
            if (status!.isSuccessfull == false) {
              ...
              openSnacbar(scaffoldKey, 'Admin Credentials Error!!');
            }
          });
        }
      });
  ...
  Future _handleUpdateComment(
    ...
    await AppService().checkInternet().then((hasInternet) async {
      if (!hasInternet!) {
        ...
        setState(() => _isSomethingChanging = true);
        await RemoteConfigService().getCredentials().then((List<String> credentials)async{
          if(credentials.isNotEmpty){
            await AuthService()
            .customAuthenticationViaJWT(credentials[0], credentials[1])
